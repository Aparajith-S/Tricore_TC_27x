/*
 * PWM_Measure.h
 * configures the GTM timer TIMx module to measure a pwm signal
...
IFX_INLINE float32 IfxGtm_Tim_In_getDutyCycle(IfxGtm_Tim_In *driver, boolean *dataCoherent)
{
    float32 duty;
    boolean interruptState = IfxCpu_disableInterrupts();
    duty          = (float32)driver->pulseLengthTick/(float32)driver->periodTick;
...
boolean IfxGtm_Tim_In_initialize(IfxGtm_Tim_In *driver, const IfxGtm_Tim_In_Config *config)
{
    boolean             result = TRUE;
...
void IfxGtm_Tim_Config(IfxGtm_Tim_In_Config *config, Ifx_GTM *gtm,IfxGtm_Tim_TinMap *port,IfxGtm_Tim_Ch chid)
{   memset(config, 0, sizeof(IfxGtm_Tim_In_Config));

    config->gtm                          = gtm;
    config->timIndex                     = IfxGtm_Tim_0;
    config->channelIndex                 = chid;
